...
class RuleGroup extends Constraints\RuleGroup {
    ...
    private function ParseExpressionTokens(EntityMap $EntityMap, $EntityVariableName, 
            array $ExpressionTokens, array $VariableMap) {
        $ExpressionSegments = $this->GetExpressionSegments($ExpressionTokens);
        $Rules = array();
        $RuleGroups = array();
        $IsAllRequired = null;
        foreach($ExpressionSegments as $SegmentTokens) {
            $BooleanOperator = array_shift($SegmentTokens);
            if($IsAllRequired === null) {
                $IsAllRequired = $BooleanOperator === T_BOOLEAN_AND;
            }
            else if($IsAllRequired && $BooleanOperator !== T_BOOLEAN_AND) {
                $RuleGroupsCopy = $RuleGroups;
                $RuleGroups = array(new Constraints\RuleGroup($Rules, $RuleGroupsCopy, $IsAllRequired));
                $IsAllRequired != $IsAllRequired;
            }
            
            if($SegmentTokens[0] === '(') {
                array_shift($SegmentTokens);
                array_pop($SegmentTokens);
                $RuleGroups[] = new RuleGroup($EntityMap, $EntityVariableName, $SegmentTokens, $VariableMap);
            }
            else {
                $Rules[] = $this->ParseRuleTokens($EntityMap, $EntityVariableName, $SegmentTokens, $VariableMap);
            }
        }
        
        return [$Rules, $RuleGroups, $IsAllRequired];
    }
    
    private function GetExpressionSegments(array $ExpressionTokens) {
        $ExpressionTokenSegments = array(array());
        $Count = 0;
        $SubExpressionCount = 0;
        foreach($ExpressionTokens as $Token) {
            if(is_array($Token) && $SubExpressionCount === 0) {
                if($Token[0] === T_BOOLEAN_AND || $Token[0] === T_BOOLEAN_OR) {
                    $Count++;
                    $ExpressionTokenSegments[$Count] = array();
                }
            }
            if($Token === '(')
                $SubExpressionCount++;
            ...
            $ExpressionTokenSegments[$Count][] = $Token;
        }
        
        return $ExpressionTokenSegments;
    }
