...
final class Map implements \IteratorAggregate, \ArrayAccess {
     ...
     * @return \Penumbra\Core\Containers\Map The created map
     * @throws \Penumbra\Core\PenumbraException If the amount of instances is not equal to the amount of to instances
     */
    public static function From(array $Instances, array $ToInstances) {
        if(count($Instances) !== count($ToInstances)) {
            throw new \Penumbra\Core\PenumbraException('Lengths of supplied arrays must be equal: %d != %d', count($Instances), count($ToInstances));
        }
     ...
     * @return Map
     * @throws \Penumbra\Core\PenumbraException If either of the parameters is not an object
     */
    public function Map($Instance, $ToInstance) {
        if(!is_object($Instance) || !is_object($ToInstance)) {
            throw new \Penumbra\Core\PenumbraException('Map only supports mapping object: %s and %s given', gettype($Instance), gettype($ToInstance));
        }
